...
function FormattedReportView({ content, tasks }: { content: string; tasks?: any[] }) {
  ...
  return (
    <div className="space-y-4 text-slate-800 dark:text-slate-100">
      ...
      {/* If no itemized tasks found, render fallback message */}
      {(!tasks || tasks.length === 0) && (
        <div className="rounded-xl border border-dashed border-border bg-muted/20 p-8 text-center text-sm text-muted-foreground">
          No itemized deliverables found for this reporting period.
        </div>
      )}
    </div>
  );
  ...
  const exportPDF = () => {
    ...
    try {
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        doc.setTextColor(148, 163, 184);
        doc.text(
          `WorkTrail • Official Work Performance Report • Page ${p} of ${totalPages}`,
          14,
